...
import InkMark from './Schema/InkMark.js';
import { editorJsToText, llmReady, museDraft, remixModes, remixPost } from './llm.js';

app.use(cors())
app.use(express.json());

app.get("/wake", (req, res) => {
    return res.status(200).json({ awake: true, message: "ok I'm up, mascara on" });
});
...
const INK_TYPES = ["ink", "coffee", "spark", "flower", "scribble"];

const formatInkMark = (mark, userId) => ({
    _id: mark._id,
    type: mark.type,
    x: mark.x,
    y: mark.y,
    rotate: mark.rotate,
    mine: userId ? String(mark.user?._id || mark.user) === String(userId) : false,
    username: mark.user?.personal_info?.username || ""
});

app.post("/ink-marks", async (req, res) => {
    try {
        const { blog_id } = req.body;
        if (!blog_id) {
            return res.status(400).json({ error: "Need a post" });
        }

        const blog = await Blog.findOne({ blog_id }).select("_id");
        if (!blog) {
        ...
        if (token) {
            try {
                userId = jwt.verify(token, process.env.SECRET_ACCESS_KEY).id;
            } catch {
                userId = null;
            }
        }

        const marks = await InkMark.find({ blog: blog._id })
            .populate("user", "personal_info.username")
            .sort({ createdAt: 1 })
            .limit(80);

        return res.status(200).json({
            marks: marks.map((mark) => formatInkMark(mark, userId)),
            mine: userId ? marks.filter((mark) => String(mark.user?._id) === String(userId)).map((mark) => mark.type) : []
        });
    } catch (err) {
        return res.status(500).json({ error: err.message });
    }
...
app.post("/leave-ink", verifyJWT, async (req, res) => {
    try {
        const user_id = req.user;
        const { _id, blog_id, type, x, y } = req.body;

        if (!INK_TYPES.includes(type)) {
            return res.status(400).json({ error: "Unknown blot" });
        }

        const blog = _id
            ? await Blog.findById(_id).select("_id author")
            : await Blog.findOne({ blog_id }).select("_id author");
        if (!blog) {
            return res.status(404).json({ error: "Post not found" });
        }

        const existing = await InkMark.findOne({ blog: _id, user: user_id, type });
        if (existing) {
            await existing.deleteOne();
            await Blog.findByIdAndUpdate(_id, { $inc: { "activity.total_inks": -1 } });
            const leftover = await InkMark.countDocuments({ blog: _id, user: user_id });
            if (!leftover) {
                await Notification.findOneAndDelete({ user: user_id, blog: _id, type: "ink" });
            }
            return res.status(200).json({ removed: true, type });
        }

        const userMarkCount = await InkMark.countDocuments({ blog: _id, user: user_id });
        if (userMarkCount >= 5) {
            return res.status(400).json({ error: "That's enough ink on this page" });
        }

        const px = Math.min(96, Math.max(4, Number(x) || 50));
        const py = Math.min(96, Math.max(4, Number(y) || 50));
        const rotate = Math.round((Math.random() * 50) - 25);

        const mark = await InkMark.create({
            blog: _id,
            user: user_id,
            type,
            x: px,
        ...
        if (String(blog.author) !== String(user_id) && userMarkCount === 0) {
        ...
        const populated = await mark.populate("user", "personal_info.username");
        return res.status(200).json({ mark: formatInkMark(populated, user_id) });
    } catch (err) {
        if (err.code === 11000) {
            return res.status(400).json({ error: "You already left that blot" });
        }
        return res.status(500).json({ error: err.message });
    }
});
...
const hitLimit = (key, max = 20, windowMs = 15 * 60 * 1000) => {
    const now = Date.now();
    const recent = (remixHits.get(key) || []).filter((time) => now - time < windowMs);
    if (recent.length >= max) return false;
    recent.push(now);
    remixHits.set(key, recent);
    return true;
};

app.get("/remix-modes", (req, res) => {
    return res.status(200).json({ modes: remixModes(), ready: llmReady() });
});

app.post("/remix-blog", async (req, res) => {
    try {
        if (!hitLimit(req.ip || "anon")) {
            return res.status(429).json({ error: "Easy — let the ink dry. Try again in a bit." });
        }

        const { blog_id, mode, question, history } = req.body;
        if (!blog_id || !mode) {
            return res.status(400).json({ error: "Need a post and a remix" });
        }

        const blog = await Blog.findOne({ blog_id }).select("title des content draft");
        if (!blog || blog.draft) {
            return res.status(404).json({ error: "Post not found" });
        }

        const text = editorJsToText(blog.content) || blog.des || "";
        if (text.length < 20) {
            return res.status(400).json({ error: "This post is too short to remix" });
        }
        ...
        return res.status(200).json({ remix, mode });
    } catch (err) {
        return res.status(err.status || 500).json({ error: err.message || "Remix failed" });
    }
});

app.post("/muse-draft", verifyJWT, async (req, res) => {
    try {
        if (!hitLimit(`muse:${req.user}`, 12)) {
            return res.status(429).json({ error: "Muse needs a sip of coffee. Try again shortly." });
        }

        const { title = "", content, focus = "all" } = req.body;
        const text = editorJsToText(content);
        const muse = await museDraft({ title, text, focus });
        return res.status(200).json(muse);
    } catch (err) {
        return res.status(err.status || 500).json({ error: err.message || "Muse failed" });
    }
});
