...
const PublishForm = () => {
  ...
  const askMuse = async () => {
    ...
    try {
      ...
      toast.success("Muse packed your post");
      setWildTitle(data.wild_title || "");
      setBlog({
        ...blog,
        title: data.title || title,
        des: data.des || des,
        tags: data.tags?.length ? data.tags.slice(0, tagLimit) : tags
      });
    } catch (err) {
      toast.dismiss(loadingToast);
      ...
      <section className='w-screen min-h-screen grid items-center lg:grid-cols-2 py-16 lg:gap-4'>
        ...
        <div className='border-grey lg:border-1 lg:pl-8'>
          <button
            ...
            onClick={askMuse}
            disabled={musing}
            className='btn-light py-2 px-5 mt-9 flex items-center gap-3 disabled:opacity-50'
          >
            <i className='fi fi-rr-magic-wand'></i>
            {musing ? "Muse is reading..." : "Let Muse pack this"}
          </button>
          <p className='text-dark-grey text-sm mt-2'>Fills title, blurb, and tags from the draft. You can still edit everything.</p>

          <p className='text-dark-grey mb-2 mt-9'>Blog title</p>
          <input type="text" placeholder='Blog Title' value={title} className='input-box pl-4 '
            onChange={handleBlogTitleChange}
          />
          {wildTitle && wildTitle !== title ?
            <button
              type="button"
          ...
          <p className='mt-1 text-dark-grey text-sm text-right'>{characterLimit - (des?.length || 0)} characters left</p>
